...
const loadObfText = [...document.querySelectorAll('[data-splitting][data-obfLoad]')];
...
loadObfText.forEach(text => {
    ...
    chars.forEach((char, position) => {
        let initialHTML = char.innerHTML;
        if(text.classList.contains("animFast")){
            gsap.fromTo(char, {
                opacity: 0
            },
            {
                ...
                onComplete: () => gsap.set(char, {innerHTML: initialHTML, delay: 0.05}),
                scrollTrigger: {
                    trigger: text,
                    start: 'top bottom',
                    end: 'bottom center',
